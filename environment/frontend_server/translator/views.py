...
def demo(request, sim_code, step, play_speed="2"): 
    ...
    # 在模拟阶段期间，我们使用 ajax 方案与前端通信步骤，
    # 对于这个演示，我们在一个步骤中发送所有移动信息。
    all_movement = dict()

    # 准备初始步骤。
    # <init_prep> 设置所有代理在演示开始时的位置和描述，
    # 由 <step> 确定。
    init_prep = dict() 
...
def process_environment(request): 
    """
    <前端到后端> 
    此函数将前端的可视化世界信息发送到后端服务器。
    它通过将当前环境表示写入 "storage/environment/{step}.json" 文件来实现。
...
def update_environment(request): 
    """
    <后端到前端> 
    此函数将后端计算的角色行为发送到前端可视化服务器。
    它通过从 "storage/movement/{step}.json" 文件中读取新的移动信息来实现。
